package org.zp.image.sample;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import org.apache.commons.codec.binary.Base64;
import org.zp.image.digest.HmacCoder;
...
public final class TokenUtil {
    ...
    public static Object testGetPolicy(String token, String tokenType) throws Exception {
        ...
        if (TokenTypeEN.UPLOAD.name().equals(tokenType)) {
            UploadPolicy policy =
                    (UploadPolicy) getPolicy(policyBase64, digestBase64, SECRET_KEY.getBytes(), tokenType);
            return policy;
        } else if (TokenTypeEN.DOWNLOAD.name().equals(tokenType)) {
            DownloadPolicy policy =
                    (DownloadPolicy) getPolicy(policyBase64, digestBase64, SECRET_KEY.getBytes(), tokenType);
            return policy;
        } else if (TokenTypeEN.MODIFY.name().equals(tokenType)) {
            ModifyPolicy policy =
                    (ModifyPolicy) getPolicy(policyBase64, digestBase64, SECRET_KEY.getBytes(), tokenType);
            return policy;
        } else {
            return null;
        }
